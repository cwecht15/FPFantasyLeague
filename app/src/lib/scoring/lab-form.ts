...
export const LAB_FIELD_GROUPS: LabFieldGroup[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "advExplosivePlay", label: "Explosive play (15+ combined)", default: DEFAULT_ADVANCED.explosivePlay ?? 0, step: 0.5, hint: "rushes or receptions of 15+ yards — or use the rush/rec splits for different thresholds (don't set both)" },
      { name: "advRushExplosive", label: "Explosive rush (10+ yd)", default: DEFAULT_ADVANCED.rushExplosive ?? 0, step: 0.5, hint: "rushes of 10+ yards — pairs with Explosive reception; leave the combined value at 0 when using the splits" },
      { name: "advRecExplosive", label: "Explosive reception (15+ yd)", default: DEFAULT_ADVANCED.recExplosive ?? 0, step: 0.5, hint: "receptions of 15+ yards" },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "coachRun2ndLong", label: "Run on 2nd & 7+", default: DEFAULT_COACHING.run2ndLong ?? 0, step: 0.25, hint: "designed run called on 2nd down with 7+ to go (scrambles/kneels excluded) — set negative to tax predictable calls" },
      { name: "coachDeep2ndShort", label: "Deep shot on 2nd & 1-2", default: DEFAULT_COACHING.deepAtt2ndShort ?? 0, step: 0.5, hint: "pass attempt of 15+ charted air yards on 2nd & short — rewards aggressive shot-taking" },
...
function ruleValue(rules: ScoringRules, name: string): number | undefined {
  ...
  const map: Record<string, number | undefined> = {
    ...
    advRushExplosive: a?.rushExplosive ?? 0,
    advRecExplosive: a?.recExplosive ?? 0,
    ...
    coachRun2ndLong: c?.run2ndLong ?? 0,
    coachDeep2ndShort: c?.deepAtt2ndShort ?? 0,
...
export function rulesFromForm(formData: FormData): ScoringRules {
  ...
  return {
    ...
    coaching: {
      ...
      run2ndLong: num(formData, "coachRun2ndLong", 0),
      deepAtt2ndShort: num(formData, "coachDeep2ndShort", 0),
    ...
    advanced: {
      ...
      rushExplosive: num(formData, "advRushExplosive", 0),
      recExplosive: num(formData, "advRecExplosive", 0),
